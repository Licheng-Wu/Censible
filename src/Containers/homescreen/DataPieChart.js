...
const DataPieChart = (props) => {
  ...
  const mapDataToPieChart = (dataObj) => {
    ...
    for (let key in dataObj) {
      if (key !== "monthlyTarget" && key !== "monthlyTotal") {
        // Filter non-category
        if (dataObj[key] > 0) {
          // Only push if category amount > 0
          results.push({
            // Push each category into results
            name: key,
    ...
    if (results.length > 5) {
      ...
      if (othersIndex === -1 || othersIndex >= 4) {
        ...
        newResult.push({
          // Creates "Others" as last category and pushes remaining amount
          name: "Others",
      ...
      // # of categories did not exceed 5
      // Add different color to pie chart
